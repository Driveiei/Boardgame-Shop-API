...
router.post('/', (req, res, next) => {
    const order = new Order({
        ...
        boardgame1: req.body.boardgame1,
        boardgame2: req.body.boardgame2,
        boardgame3: req.body.boardgame3,
        boardgame4: req.body.boardgame4,
        boardgame5: req.body.boardgame5
    });
    order
        .save()
        .then(doc => {
...
router.get('/findByYear/:orderYear', (req, res, next) => {
...
//year cashier
router.get('/findByYC/:orderDetails', (req, res, next) => {
    const order = JSON.parse(req.params.orderDetails);
    const year = order.year;
    const cashier = order.cashier;
    
    Order.find({ $and: [ {cashier_name: cashier},
                {year: year} ]
                })
     .exec()
     .then(docs => {
